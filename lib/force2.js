...
class Force {
    ...
    async queryTestClasses(csvFilePath) {
        var changeFileNames = ['LicenceUtility'];
        let siblingTestClassArray = [];
        
        var extractedData = "";
        var parser = new xml2js.Parser();
        var xml = fs.readFileSync("C:/Workplace/SFDX4/package/package.xml", "utf8");
            ...
            for(var f=0; f<changeFileNames.length; f++){
                ...
                for (var k=0; k < result.records.length; k++) {
                    ...
                    if((record.RefMetadataComponentName == changeFileName) && record.MetadataComponentName.includes('Test')){
                        ...
                        // console.log('ClassName: ' + record.RefMetadataComponentName);
                        // console.log('Dependency: ' + record.MetadataComponentName);
                        siblingTestClassArray.push(record.MetadataComponentName);
                    }
                }
            }
            // console.log('sibling test classes ' + siblingTestClassArray.join(","));
            // fs.writeFileSync(csvFileOut, siblingTestClassArray.join('\n'));
            
            
        });
        
        console.log('sibling test classes ' );
        //await fs.writeFile('output/allTestClasses.csv', siblingTestClassArray.toString() ,function(error){ console.log('error occuring')});
        
        // var  file = fs.createWriteStream('output/allTestClasses.csv');
        // file.write('eafdsdfsdfsd');
        // // siblingTestClassArray.forEach(value => console.log(value)); 
        // file.on('error', function(err) {  /* error handling */ 
        //         console.error('There is an error writing the file ${csvFileOut} => ${err}')
        //     });
        // file.on('finish', () => {
        //         console.log('wrote all the array data to file ${csvFileOut}');
        //         });
        // file.end();
    }
